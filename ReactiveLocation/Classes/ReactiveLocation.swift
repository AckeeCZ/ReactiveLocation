...
func < (lhs: LocationAuthorizationLevel, rhs: LocationAuthorizationLevel) -> Bool {
    return lhs == .WhenInUse && rhs == .Always
}
extension LocationAuthorizationLevel: Comparable { }

extension LocationAuthorizationLevel {
...
enum LocationAuthorizationError: ErrorType {
    case Denied
    case Restricted
}
...
class LocationManager : LocationManaging {
    ...
    lazy var authorizeAction: Action<LocationAuthorizationLevel, LocationAuthorizationLevel, LocationAuthorizationError> = { [unowned self] in
